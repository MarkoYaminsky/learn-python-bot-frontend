...
def submit_homework(student_id: int, topic: str, content: str):
    request = GraphqlRequest.form_request(
        ...
        query_params=[
            QueryParameter(parameter_name="studentId", parameter_value=student_id),
            QueryParameter(parameter_name="topic", parameter_value=topic),
            QueryParameter(parameter_name="content", parameter_value=content),
        ],
    )
...
def add_homework(topic: str, task: str):
    request = GraphqlRequest.form_request(
        ...
        query_params=[
            QueryParameter(parameter_name="topic", parameter_value=topic),
            QueryParameter(parameter_name="task", parameter_value=task),
        ],
    )
    request_details = GraphqlRequest(request).send_request().get("requestDetails", None)
    return request_details
...
def assign_homework(option: str, option_value: str, topic: str):
    request = GraphqlRequest.form_request(
        ...
        query_params=[
            QueryParameter(parameter_name=option, parameter_value=option_value),
            QueryParameter(parameter_name="homeworkTopic", parameter_value=topic),
        ],
    )
    request_details = GraphqlRequest(request).send_request().get("requestDetails", None)
    return request_details
...
def get_submitted_homework():
    request = GraphqlRequest.form_request(
        ...
        output_fields=[
            "content",
            OutputField(
                parrent_name="student", nested_fields=["telegramId", "username"]
            ),
            OutputField(parrent_name="homework", nested_fields=["id", "task"]),
        ],
    )
    data = GraphqlRequest(request).send_request()
    df_values = pd.DataFrame(data).values
    return [
        {"content": content, "student": student, "homework": homework}
        for content, student, homework in df_values
    ]
...
def approve_or_decline_homework(option: str, student_id: int, homework_id: int):
    ...
    request = GraphqlRequest.form_request(
        ...
        query_params=[
            QueryParameter(parameter_name="studentId", parameter_value=student_id),
            QueryParameter(parameter_name="homeworkId", parameter_value=homework_id),
        ],
    )
