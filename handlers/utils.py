...
def assignment_with_option(option: str, from_user_id: int):
    ...
    def get_topic(topic_message: types.Message, user_or_group):
        ...
        assignment_details = assign_homework(option=query_params[option], option_value=user_or_group, topic=topic)
    ...
    to_who = 'групу, якій' if option == 'group' else 'псевдонім студента, якому'
    sent_user_or_group = bot.send_message(from_user_id, f'Введіть {to_who} хочете призначити домашнє завдання.')
