...
class GraphqlRequest:
    ...
    def query_name(self):
        ...
        return line_with_request_name.split("(" if request_is_parametrized else "{")[
            0
        ].strip()
    ...
    def __format_query_parameters(query_params: list[QueryParameter]) -> str:
        ...
        formatted_query_params = ", ".join(
            [
                f"{query_param.parameter_name}: {query_param.parameter_value}"
                for query_param in query_params
            ]
        )
        ...
        output_fields = (
            " ".join(
                [
                    ...
                    else f"{field.parrent_name} "
                    + "{"
                    + f" {' '.join(field.nested_fields)} "
                    + "}"
    ...
    def send_request(self) -> dict:
        return requests.post(SERVER_URL, json={"query": self.query}).json()["data"][
            self.query_name
        ]
