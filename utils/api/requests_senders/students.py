...
def get_students_by_group_name(group_name: str):
    ...
    return [{'username': username, 'id': telegram_id} for username, telegram_id in df_values]
...
def get_student_username(student_id: int):
    request = GraphqlRequest.form_request(
        ...
        query_params=[QueryParameter(parameter_name="telegramId", parameter_value=student_id)],
...
def get_student_id(username: str):
    request = GraphqlRequest.form_request(
        request_type=RequestType.QUERY,
        instance_name='student',
        query_params=[QueryParameter(parameter_name="username", parameter_value=username)],
...
def register_student(telegram_id: int, username: str):
    request = GraphqlRequest.form_request(
        ...
        query_params=[
            QueryParameter(
                parameter_name='telegramId',
                parameter_value=telegram_id),
            QueryParameter(
